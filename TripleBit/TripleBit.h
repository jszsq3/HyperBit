...
struct ChunkTriple {
	...
	ChunkTriple(){}
	ChunkTriple(ID subjectID, ID predicateID, ID objectID,ID timeID, TripleNode::Op operation):
		subjectID(subjectID), predicateID(predicateID), objectID(objectID),timeID(timeID), operation(operation){}
	ChunkTriple(ID subjectID, ID objectID, ID timeID,TripleNode::Op operation):
