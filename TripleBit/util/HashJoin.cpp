...
void HashJoin::SortMergeJoin(ID* buffer1, ID* buffer2, int IDCount1, int IDCount2, int joinKey1, int joinKey2, int size1, int size2, char* flagVector1, char* flagVector2) {
	int i, j;
	ID keyValue;
...
void HashJoin::SortMergeJoin(ID* buffer1, ID* buffer2, int IDCount1, int IDCount2, int joinKey1, int joinKey2, int size1, int size2, char* flagVector1) {
	int i, j;
	ID keyValue;
...
//	/*
	if (arg->length2 > 100 && arg->length1 > 100) {
		...
		HashJoin::BuildHashIndex(arg->buffer1->getBuffer() + arg->startPos1 * arg->buffer1->IDCount, arg->joinKey1, hashKey, hashBucket1, prefixSum1, arg->length1, arg->buffer1->IDCount);
		HashJoin::BuildHashIndex(arg->buffer2->getBuffer() + arg->startPos2 * arg->buffer2->IDCount, arg->joinKey2, hashKey, hashBucket2, prefixSum2, arg->length2, arg->buffer2->IDCount);
		...
		for (size_t i = 0; i < hashKey; i++) {
			if (hashBucket1[i] != 0 && hashBucket2[i] != 0) {
				...
				if (arg->flag2 != NULL)
					HashJoin::SortMergeJoin(arg->buffer1->getBuffer() + (arg->startPos1 + start1) * arg->buffer1->IDCount, arg->buffer2->getBuffer() + (arg->startPos2 + start2) * arg->buffer2->IDCount, arg->buffer1->IDCount, arg->buffer2->IDCount, arg->joinKey1, arg->joinKey2, size1, size2, arg->flag1 + arg->startPos1 + start1, arg->flag2 + arg->startPos2 + start2);
				else
					HashJoin::SortMergeJoin(arg->buffer1->getBuffer() + (arg->startPos1 + start1) * arg->buffer1->IDCount, arg->buffer2->getBuffer() + (arg->startPos2 + start2) * arg->buffer2->IDCount, arg->buffer1->IDCount, arg->buffer2->IDCount, arg->joinKey1, arg->joinKey2, size1, size2, arg->flag1 + arg->startPos1 + start1);
			}
		}
...
//	*/
		if (arg->flag2 != NULL)
			HashJoin::SortMergeJoin(arg->buffer1->getBuffer() + arg->startPos1 * arg->buffer1->IDCount, arg->buffer2->getBuffer() + arg->startPos2 * arg->buffer2->IDCount, arg->buffer1->IDCount, arg->buffer2->IDCount, arg->joinKey1, arg->joinKey2, arg->length1, arg->length2, arg->flag1 + arg->startPos1, arg->flag2 + arg->startPos2);
		else
			HashJoin::SortMergeJoin(arg->buffer1->getBuffer() + arg->startPos1 * arg->buffer1->IDCount, arg->buffer2->getBuffer() + arg->startPos2 * arg->buffer2->IDCount, arg->buffer1->IDCount, arg->buffer2->IDCount, arg->joinKey1, arg->joinKey2, arg->length1, arg->length2, arg->flag1 + arg->startPos1);
	}
...
void HashJoin::BuildHashIndex(ID* buffer, int joinKey, ID hashKey, vector<int>& hashBucket, vector<int>& prefixSum, int size, int IDCount) {
	int i = 0;
...
void HashJoin::Join(EntityIDBuffer* entBuffer1, EntityIDBuffer* entBuffer2, int joinKey1, int joinKey2) {
	joinKey1--;
	joinKey2--;
...
//	cout<<"iSize: "<<size1<<" jSize: "<<size2<<endl; 
	...
	BuildHashIndex(entBuffer1->getBuffer(), joinKey1, hashKey, entBuffer1->hashBucket, entBuffer1->prefixSum, entBuffer1->getSize(), entBuffer1->IDCount);
	BuildHashIndex(entBuffer2->getBuffer(), joinKey2, hashKey, entBuffer2->hashBucket, entBuffer2->prefixSum, entBuffer2->getSize(), entBuffer2->IDCount);
	...
	for (i = 0; i < hashKey; i++) {
		if (entBuffer1->hashBucket[i] != 0 && entBuffer2->hashBucket[i] != 0) {
			...
			ThreadPool::getWorkPool()[rand()%2]->addTask(boost::bind(&HashJoin::run, arg));
		}
	}
