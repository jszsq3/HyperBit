...
enum LengthType {
	LengthType_UChar = 0, LengthType_UShort = 2, LengthType_UInt = 4, LengthType_ULongLong = 6
};

const int LengthTypeMask = 6;

enum ObjectPoolType {
	ObjectPoolType_Normal = 1 << 4, ObjectPoolType_Fixed = 2 << 4,
};
...
public:
	...
	//添加<ID,str>到stringPool，返回插入数据的偏移
	OffsetType append_object_with_id(ID id, OffsetType length, const void * pdata) {
		...
		// TODO: check length
		ObjectPoolMeta * meta = get_meta();
		if (meta->usage + alllength > meta->length) {
#ifdef USE_C_STRING
			OffsetType new_length = expand(meta->length, alllength, meta->classtype);
#else
		...
		uchar * top = data->get_address() + meta->usage;

#ifdef USE_C_STRING
		*(ID*) top = id;
		memcpy(top + sizeof(ID), pdata, length);
		*(top + sizeof(ID) + length) = '\0';
		OffsetType ret = meta->usage;
		meta->usage += alllength;
		meta->size++;
#else
