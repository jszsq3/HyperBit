...
QuerySemanticAnalysis::QuerySemanticAnalysis(IRepository &repo) :
		repo(repo) {
}
...
// Encode a triple pattern for graph. encode subject,predicate,object to ids, also store their types.
	...
	case SPARQLParser::Element::Variable:
		tripleNode.objectID = triplePattern.object.id;
		tripleNode.constObject = false;
		break;
	case SPARQLParser::Element::Constant:
		switch (triplePattern.object.dataType) {
		case BOOL:
		case CHAR:
		case INT:
		case UNSIGNED_INT:
		...
		case DATE:
		case DOUBLE:
			tripleNode.objectID = triplePattern.object.dValue;
...
//collect all variables id from triple nodes.
	...
	for (unsigned int index = 0, limit = query.tripleNodes.size(); index < limit; index++) {
		...
		if (!tpn.constObject) {
			if (isUnused(query, tpn, tpn.objectID) == false || query.tripleNodes.size() == 1) {
				iter = find(query.joinVariables.begin(), query.joinVariables.end(), tpn.objectID);
				if (iter == query.joinVariables.end())
					query.joinVariables.push_back(tpn.objectID);
			}
		}
	}
	...
	for (unsigned int i = 0; i < limit; i++) {
		...
		for (unsigned int j = 0; j < triplenodes_size; ++j) {
			...
			if (!query.tripleNodes[j].constObject) {
				if (query.joinVariableNodes[i].value == query.tripleNodes[j].objectID) {
					//add an edge
					std::pair<TripleBitQueryGraph::TripleNodeID, TripleBitQueryGraph::JoinVariableNode::DimType> edge;
					edge.first = query.tripleNodes[j].tripleNodeID;
...
// Transform a subquery, encoding PatternGroup, fill the subquery,build the JoinVariable Nodes and their edges with triple nodes.
	...
	if (!encodeJoinVariableNodes(output))
		return false;
...
static bool transformQuery(IRepository& repo, const SPARQLParser& input, TripleBitQueryGraph& output) {
	...
	case SPARQLParser::Modifier_None:
		output.setDuplicateHandling(TripleBitQueryGraph::AllDuplicates);
		break;
	case SPARQLParser::Modifier_Distinct:
		output.setDuplicateHandling(TripleBitQueryGraph::NoDuplicates);
		break;
	case SPARQLParser::Modifier_Reduced:
		output.setDuplicateHandling(TripleBitQueryGraph::ReducedDuplicates);
		break;
	case SPARQLParser::Modifier_Count:
		output.setDuplicateHandling(TripleBitQueryGraph::CountDuplicates);
		break;
	case SPARQLParser::Modifier_Duplicates:
		output.setDuplicateHandling(TripleBitQueryGraph::ShowDuplicates);
		break;
	}
