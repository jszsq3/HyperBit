...
StringIDSegment::StringIDSegment() :
		fillRate(0.5), stringPool(NULL), stringHashTable(NULL), idStroffPool(NULL) {
...
StringIDSegment* StringIDSegment::create(const string dir, const string segmentName) {
	...
	segment->stringPool = ObjectPool<uint>::create(LengthType_UInt | ObjectPoolType_Normal, new_path.c_str(), 1 << 16);
	...
	segment->idStroffPool = FixedObjectPool::create(ObjectPoolType_Fixed, new_path.c_str(), 1 << 16, sizeof(IDStroffEntry));
...
ID StringIDSegment::addStringToSegment(LengthString * aStr) {
	...
	OffsetType strOff = addStringToStringPoolAndUpdateStringHashTable(aStr, nextId);//更新stringPool 、 StringHashTable 

	if (strOff) {
		IDStroffEntry ise;
		//ise.id = nextId;
		ise.stroff = strOff;
		ID id = addIDStroffToIdStroffPool(&ise); //更新id_stroffPool
		assert(id == nextId);
		return id;
	}
	// TODO: none id?
