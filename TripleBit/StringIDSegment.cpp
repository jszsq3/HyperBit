...
ID StringIDSegment::addStringToSegment(LengthString *aStr)
{
	...
	//strOff返回aStr在StringPool的偏移量
	OffsetType strOff = addStringToStringPoolAndUpdateStringHashTable(aStr, nextId); //����stringPool �� StringHashTable
	...
	if (strOff)
	{
		...
		// ise.id = nextId;
		ise.stroff = strOff;
		ID id = addIDStroffToIdStroffPool(&ise); //����id_stroffPool
		assert(id == nextId);
		return id;
	}
	// TODO: none id?
